<template>
  <div>
    <calendar-options :selected-day="realSelectedDay" :state-options="stateOptions" v-bind="$attrs" v-on="$listeners" />
    <div class="el-calendar">
      ...
      <div class="el-calendar__body" v-if="validatedRange.length === 0" key="no-range">
        <date-table :date="date" :selected-day="realSelectedDay" :first-day-of-week="realFirstDayOfWeek" @pick="pickDay"
          :state-options="stateOptions" v-bind="$attrs" v-on="$listeners" />
        <div>
          <div v-for="state of stateList" :key="state.stateId" class="contain-flex">
            <div class="table-cell-box" :style="state | renderCellStyle" :class="state.className">{{ state.boxContent }}
            </div>
      ...
      <div v-else class="el-calendar__body" key="has-range">
        <date-table v-for="(range, index) in validatedRange" :key="index" :date="range[0]"
          :selected-day="realSelectedDay" :range="range" :hide-header="index !== 0" @pick="pickDay"
          :state-options="stateOptions" v-bind="$attrs" v-on="$listeners" />
        <div>
          <div v-for="state of stateList" :key="state.stateId" class="contain-flex">
            <div class="table-cell-box" :style="state | renderCellStyle" :class="state.className">{{ state.boxContent }}
